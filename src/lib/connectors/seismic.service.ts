/**
 * @fileOverview Connector for Seismic.
...
class SeismicService {
    async sync(source: DataSource) {
        console.log(`Starting mock sync for Seismic source: ${source.name}`);
        ...
        console.log(`Finished mock sync for Seismic source: ${source.name}`);
        return source;
    }
}
