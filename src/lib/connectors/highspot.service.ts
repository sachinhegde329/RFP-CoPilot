/**
 * @fileOverview Connector for Highspot.
...
class HighspotService {
    async sync(source: DataSource) {
        console.log(`Starting mock sync for Highspot source: ${source.name}`);
        ...
        console.log(`Finished mock sync for Highspot source: ${source.name}`);
        return source;
    }
}

export const highspotService = new HighspotService();
